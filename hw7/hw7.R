...
names<-as.matrix(unique(presidents))
prespeeches<-apply(names,1,function(x) which(speechesDF$Pres==x, arr.ind=T))
presidentspeech<-lapply(prespeeches, function(x) wordMat[,x])
n<-length(names)
presidentWordMat<-sapply(c(1:n), 
                        function(x){apply(as.matrix(presidentspeech[[x]]),1,sum)}) 
...
# col = cols[presParty[rownames(presDist)]]
  ...
  plot(mds, type = "n", xlab = "x", ylab = "y", main="Presidents")
...
plot(speechesDF$year, speechesDF$char/speechesDF$word, xlab="speech year", ylab="average word length")
plot(speechesDF$year, speechesDF$word/speechesDF$sent, xlab="speech year", ylab="average sentence length")
