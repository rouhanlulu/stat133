...
# record in the men's 1500 meter.
 ...
 wr.name <-wr1500m[wr1500m$year==max(wr1500m$year),]$athlete
...
wr1500m <- transform(wr1500m,times_sec=wr1500m$times+180)
...
text(wr_1944,wr1500m$times_sec[wr1500m$year==1944],wr1500m$athelete[wr1500m$year==1944])
text(wr_1998,wr1500m$times_sec[wr1500m$year==1998],wr1500m$athelete[wr1500m$year==1998])
...
plot(wr1500m$new_year,wr1500m$times_sec,type="s",xlim=c(1892,2015),xlab = "Year",
...
points(x = c(wr_1998, 2012+9/12),
       ...
       type = "l")
text(wr_1944-0.5,wr1500m$times_sec[wr1500m$year==1944],wr1500m$athlete[wr1500m$year==1944],col="gold")
text(wr_1998-0.5,wr1500m$times_sec[wr1500m$year==1998],wr1500m$athlete[wr1500m$year==1998],col="green")
...
symbols(log(SO2012Ctry$pop), log(SO2012Ctry$GDP_per_person), 
               circles = sqrt(SO2012Ctry$Total)/80, inches = 0.2)
...
with(SO2012Ctry[SO2012Ctry$Total > 0, ], 
     symbols(log(pop), log(GDP_per_person), 
             circles= sqrt(Total)/80, inches = 0.2))
with(SO2012Ctry[SO2012Ctry$Total == 0, ], 
...
text(log(SO2012Ctry$pop)[top5]+0.28, log(SO2012Ctry$GDP_per_person)[top5], 
     labels = SO2012Ctry$ISO[top5],adj=1,
     cex = seq(.7, .3, by = -.2), col = "blue")
...
barplot(table(athletes$Sex, athletes$Sport)[ , orderSport],
        beside = TRUE, cex.names = 1, las = 3 , ylim = c(0,1400),
