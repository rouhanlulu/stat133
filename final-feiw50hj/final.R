...
name <- "ROUHAN GUO"
github.name <- "rouhanlulu"
email.address <- "rouhan1995@berkeley.edu"
...
# i.e. each element is one of the numbers 1 through 6, with equal probability.
  set.seed(31415)
v <- sample(c(1,2,3,4,5,6),replace=TRUE,100)
...
set.seed(2718)
w <- c("aa", "ab", "ac", "ad", "ae","af","ag","ah","ai","aj","ak","al","am","an","ao","ap","aq","
...
set.seed(344)
m <- matrix(rnorm(100,mean=8),nrow=10,byrow=FALSE)
...
#  
bdiff <-abs(mean(infants$bwt[infants$age>=35],na.rm=TRUE)-mean(infants$bwt[infants$age<35],na.rm=TRUE))
...
plot(WorldBank$"life.expectancy",WorldBank$"fertility.rate",xlab="life expectancy",ylab="fertility",pch=20,col=as.numeric(WorldBank$region))
abline(h=2.1)
# [4 pts]
...
par(mfrow=c(1,2))
plot(WorldBank$"life.expectancy"[WorldBank$year==1960],WorldBank$"fertility.rate"[WorldBank$year==1960],xlab="life expectancy 1960",ylab="fertility 1960",pch=20,col=as.numeric(WorldBank$region))
plot(WorldBank$"life.expectancy"[WorldBank$year==2013],WorldBank$"fertility.rate"[WorldBank$year==2013],xlab="life expectancy 2013",ylab="fertility 2013",pch=20,col=as.numeric(WorldBank$region))
...
par(mfrow=c(1,1))
hist(WorldBank$"GDP.per.capita.Current.USD"[WorldBank$lending=="IDA"],xlab="GDP",main="GDP FOR IDA lending",breaks=seq(from=0,by=250,length.out=30))
...
prop.rain <- sapply(rain,function(x) sum(rain[[i]]>0))
...
#    mean(cases)-mean(controls) for one permutation
 ...
 PermDiff <- function(cases, controls, k=5000){
         for (i in 1:5000)
         com<-c(cases,controls)
         cases<-sample(com,size=length(cases),replace=FALSE) 
         controls<-subset(com,select=cases)
         meanv[i]<-mean(cases)-mean(controls)
...
#  <your code here>
   for(i in 1:B)
        winners<-0
        winningnumbers<-sample(1:19,size=3,replace=F)
        for (j in 1:k)
            a<-sample(1:19,size=3,replace=F)
            if (a==winningnumbers) winners<-winners+1
...
text0<- grep("st",phrases)
text00<- grep("St",phrases)
text1<-c(text0,text00)
...
match10 <- grep("AGT",dna)
match11 <- grep("GGT",dna)
match1<-c(match10,match11)
